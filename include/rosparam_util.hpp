...
namespace CRSLib
{
    namespace RosparamUtil
    {
        ...
        inline Param get_param(const Param& xml_opt, const int index, bool supress_out_of_range_error = false) noexcept
        {
            ...
            {
                if(!supress_out_of_range_error) ROS_ERROR("Stew: out range access of %s.", xml_opt->param_name.c_str());
                return std::nullopt;
            }
        ...
        template<class T>
        inline std::optional<T> nullable_cast(const Param& xml_opt) noexcept
        {
            if(!xml_opt.has_value())
            {
                return std::nullopt;
